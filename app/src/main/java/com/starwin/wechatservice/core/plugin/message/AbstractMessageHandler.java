...
import com.starwin.wechatservice.core.plugin.Storage;
import com.starwin.wechatservice.core.plugin.TulingRobot;
import com.starwin.wechatservice.core.plugin.TxtMessageRobot;
...
public abstract class AbstractMessageHandler implements IMessageHandler, TxtMessageRobot {
	...
	public boolean download(JSONObject msg, Enums.MsgType msgType) {
		...
		String ext = "";
		if (msgType == Enums.MsgType.PICTURE) {
			ext = "jpg";
		} else if (msgType == Enums.MsgType.VOICE) {
			ext = "mp3";
		} else if (msgType == Enums.MsgType.VIDEO || msgType == Enums.MsgType.SMALL_VIDEO) {
