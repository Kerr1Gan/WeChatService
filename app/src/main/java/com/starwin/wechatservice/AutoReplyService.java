...
public class AutoReplyService extends AccessibilityService {
    ...
    public void onAccessibilityEvent(final AccessibilityEvent event) {
        ...
        switch (eventType) {
                    ...
                    for (CharSequence text : texts) {
                        ...
                        if (!TextUtils.isEmpty(content)) {
                            if (isScreenLocked()) {
                                locked = true;
                                wakeAndUnlock();
                                android.util.Log.i("maptrix", "the screen is locked");
                                if (isAppForeground(MM_PNAME)) {
                                    ...
                                    background = true;
                                    android.util.Log.i("maptrix", "is mm in background");
                                    sendNotifacationReply(event);
                                }
                            } else {
                                locked = false;
                                android.util.Log.i("maptrix", "the screen is unlocked");
                                if (isAppForeground(MM_PNAME)) {
                                    background = false;
                                    android.util.Log.i("maptrix", "is mm in foreground");
                                    sendNotifacationReply(event);
            ...
            case AccessibilityEvent.TYPE_WINDOW_STATE_CHANGED:
                android.util.Log.i("maptrix", "get type window down event");
                if (!hasAction) break;
                itemNodeinfo = null;
    ...
    private void release() {
        ...
        if (locked && kl != null) {
            android.util.Log.i("maptrix", "release the lock");
            //得到键盘锁管理器对象
            kl.reenableKeyguard();
            locked = false;
        }
